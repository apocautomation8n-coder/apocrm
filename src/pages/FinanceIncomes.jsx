...
export default function FinanceIncomes({ hideHeader = false }) {
  ...
  const fetchData = async () => {
    ...
    // Fetch project payments (type = 'ingreso' and project_id is not null) 
  ...
  const handleSave = async () => {
    ...
    const payload = { 
      ...
      description: form.description || `Pago de ${projects.find(p => p.id === form.project_id)?.client || 'cliente'}`,
      date: form.date,
  ...
  const handleDelete = async (i) => {
    if (!confirm('¿Eliminar este registro de ingreso? (Nota: Los saldos bancarios y del proyecto no se revertirán automáticamente)')) return
          ...
          <div className="overflow-x-auto">
                  ...
                  <tr key={i.id} className="border-b border-surface-800/30 hover:bg-surface-800/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex flex-col">
                        <span className="text-surface-200 font-medium">{i.project?.client || 'Venta Directa'}</span>
            ...
            {filtered.length === 0 && (
              <div className="p-12 text-center text-surface-500">
                No hay ingresos registrados con estos criterios.
              </div>
            )}
          </div>
        )}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Registrar Cobro / Ingreso">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            <Select label="Proyecto Vinculado (Opcional)" value={form.project_id} onChange={(e) => setForm(f => ({ ...f, project_id: e.target.value }))}>
              <option value="">Venta Directa / Otro</option>
              {projects.map(p => (
                <option key={p.id} value={p.id}>{p.client} (Pendiente: {p.remaining})</option>
              ))}
            </Select>
            <Select label="Cuenta de Destino (Caja)" value={form.bank_account_id} onChange={(e) => setForm(f => ({ ...f, bank_account_id: e.target.value }))}>
          ...
          <Input label="Monto a Ingresar" type="number" value={form.amount} onChange={(e) => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="0.00" />
          <Input label="Descripción Corta" value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Pago cuota 1, Adelanto, etc." />
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-surface-800">
            ...
            <Button onClick={handleSave} className="px-8">Registrar e Impactar Saldos</Button>
          </div>
        </div>
      </Modal>
    </div>
